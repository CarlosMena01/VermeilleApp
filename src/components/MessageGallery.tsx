...
const MessageGallery: React.FC<MessageGalleryProps> = ({ onFinish }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-rose-50 p-6">
            ...
            <div className="relative w-full max-w-sm aspect-[4/5] bg-white rounded-2xl shadow-xl p-8 flex flex-col justify-center items-center text-center transition-all duration-300 transform hover:scale-[1.02]">
            ...
            <div className="flex justify-between w-full max-w-sm mt-8 px-4">
                <button
                    ...
                    className={`p-3 rounded-full ${currentIndex === 0 ? 'text-gray-300' : 'text-pink-500 bg-white shadow-md hover:bg-pink-50'}`}
                >
                    <ChevronLeft />
                </button>
                ...
                >
                    {currentIndex === messages.length - 1 ? 'Sorpresa Final ✨' : 'Siguiente'}
                    {currentIndex !== messages.length - 1 && <ChevronRight size={20} />}
                </button>
            </div>
        </div>
    );
